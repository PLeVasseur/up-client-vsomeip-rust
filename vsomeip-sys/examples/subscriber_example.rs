...
use vsomeip_sys::safe_glue::{get_pinned_application, get_pinned_message_base, get_pinned_payload, get_pinned_runtime, register_availability_handler_fn_ptr_safe, register_message_handler_fn_ptr_safe, request_single_event_safe, set_data_safe};
use vsomeip_sys::vsomeip;
use vsomeip_sys::vsomeip::{ANY_MAJOR, ANY_MINOR, instance_t, message, runtime, service_t};
...
fn main() {
    ...
    request_single_event_safe(&mut app_wrapper, SAMPLE_SERVICE_ID, SAMPLE_INSTANCE_ID, SAMPLE_EVENT_ID, SAMPLE_EVENTGROUP_ID);
    get_pinned_application(&app_wrapper).subscribe(SAMPLE_SERVICE_ID, SAMPLE_INSTANCE_ID, SAMPLE_EVENTGROUP_ID, ANY_MAJOR, SAMPLE_EVENT_ID);
